...
import { MENU_PERMISSION_MAP, extractCategoriesFromPermissions } from "@/lib/auth/permissions";
...
interface NavItem {
  ...
  icon: LucideIcon;
  badge?: string | number;
  requiresAdmin?: boolean; // 관리자 전용 메뉴 항목
}
...
interface NavSection {
  ...
  requiresHeadquarters?: boolean; // 본사 전용 (협력사 접근 불가)
  requiredResource?: string; // 섹션에 필요한 권한
  items?: NavItem[];
...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    items: [
      ...
      { id: "messages", label: "메시지함", href: "/dashboard/messages", icon: Mail },
    ],
  },
  {
    id: "chat",
  ...
  },
  {
    ...
    title: "주문 관리",
    items: [
      { id: "orders-main", label: "주문 데이터 통합(고객상세조회)", href: "/dashboard/orders", icon: ShoppingCart },
      { id: "orders-status", label: "주문상태확인(협력사)", href: "/dashboard/orders/status", icon: RefreshCcw },
  ...
  },
  {
    ...
    title: "AS 관리",
    items: [
      { id: "as-main", label: "AS 접수 및 관리", href: "/dashboard/after-service", icon: Settings },
      { id: "as-kpi", label: "AS KPI 대시보드", href: "/dashboard/after-service/kpi", icon: BarChart3 },
      { id: "as-insights", label: "AS 인사이트 (LLM)", href: "/dashboard/after-service/insights", icon: Bot },
    ],
  },
  {
    id: "marketing",
  ...
  },
  {
    ...
    title: "성과 관리",
    items: [
      { id: "perf-integrated", label: "통합 대시보드", href: "/dashboard/performance/integrated", icon: BarChart2, requiresAdmin: true },
      { id: "perf-kpi", label: "실시간 KPI 대시보드", href: "/dashboard/performance/kpi", icon: PieChart },
      { id: "perf-products", label: "상품별 성과", href: "/dashboard/performance/products", icon: Package },
      ...
      { id: "perf-statistics", label: "협력사 통계", href: "/dashboard/performance/statistics", icon: BarChart3 },
    ],
  },
  {
    id: "alerts",
  ...
  },
  {
    ...
    items: [
      ...
      { id: "data-trash", label: "휴지통", href: "/dashboard/data/trash", icon: Trash2 },
    ],
  },
  {
    id: "reports",
  ...
  },
  {
    ...
    title: "쇼핑몰 관리",
    items: [
      { id: "mall-home", label: "쇼핑몰 바로가기", href: "/mall", icon: Store },
      { id: "mall-products", label: "상품 관리", href: "/dashboard/mall/products", icon: Package },
      { id: "mall-orders", label: "쇼핑몰 주문", href: "/dashboard/mall/orders", icon: ShoppingCart },
      { id: "mall-users", label: "회원 관리", href: "/dashboard/mall/users", icon: Users },
      { id: "mall-qna", label: "Q&A 관리", href: "/dashboard/mall/qna", icon: HelpCircle },
    ],
  },
  {
    id: "custom",
    title: "사용자 정의",
    isCollapsible: true,
    requiresHeadquarters: true, // 본사 전용 - 협력사 접근 불가
    subSections: [
      {
        id: "custom-analysis",
        label: "고객 분석",
      ...
      },
      {
        ...
        items: [
          ...
          { id: "chat-history", label: "상담내역저장", href: "/dashboard/support/chat-history", icon: MessageSquare },
        ],
      },
      {
        id: "custom-partners",
        label: "파트너",
        items: [
          { id: "partners-portal", label: "파트너 포털", href: "/dashboard/partners/portal", icon: Users },
          { id: "partners-edu", label: "교육 및 가이드", href: "/dashboard/partners/education", icon: BookOpen },
  ...
  },
  {
    ...
    requiresAdmin: true,
    items: [
      { id: "admin-users", label: "사용자 관리", href: "/dashboard/users", icon: Users },
      { id: "admin-roles", label: "역할 및 권한", href: "/dashboard/roles", icon: Shield },
...
function canAccessMenuItem(
  ...
  if (!requiredResource) return true; // 매핑되지 않은 메뉴는 기본 접근 허용
  
  // 1. 세밀한 권한(예: as_management:request_management)이 요구되는 경우
  if (requiredResource.includes(':')) {
    // 정확히 일치하는 세밀한 권한이 있는지 체크
    return userPermissions.some(p => p.resource === requiredResource);
  }
  
  // 2. 카테고리 권한(예: as_management)이 요구되는 경우
  // 세밀한 권한을 카테고리로 변환하여 체크
  const userCategories = extractCategoriesFromPermissions(userPermissions);
  return userCategories.has(requiredResource);
}
...
function filterItemsByPermission(
  ...
  return items.filter(item => {
    // 관리자 전용 항목인 경우 ADMIN만 접근 가능
    if (item.requiresAdmin && userRole !== "ADMIN") {
      return false;
    }
    return canAccessMenuItem(userPermissions, item.id, userRole);
  });
}

function filterSubSectionsByPermission(
  subSections: NavSubSection[],
  userPermissions: Array<{resource: string}>,
  userRole?: string
): NavSubSection[] {
  return subSections
    .map(subSection => ({
      ...subSection,
      items: filterItemsByPermission(subSection.items, userPermissions, userRole)
    }))
...
export function Sidebar({ className, userRole = "USER", isCollapsed = false, userPermissions = [], isPartner = false }: SidebarProps) {
  ...
  const filteredSections = useMemo(() => {
    // ADMIN은 모든 섹션 표시 (협력사 전용 제외)
    if (userRole === "ADMIN") {
      return NAV_SECTIONS
        .map((section) => {
          // 본사 전용 섹션은 협력사 사용자 접근 불가
          if (section.requiresHeadquarters && isPartner) {
            return null;
          }
          return section;
        })
    ...
    return NAV_SECTIONS
      .map((section) => {
        ...
        // 시스템 관리 섹션은 system_management 권한 필요
        if (section.requiresAdmin) {
          if (!userCategories.has('system_management')) return null;
        }

        // 일반 섹션: 아이템들을 권한에 따라 필터링
        if (section.items) {
          const filteredItems = filterItemsByPermission(section.items, userPermissions, userRole);
          if (filteredItems.length === 0 && !section.isMain) return null;
          return { ...section, items: filteredItems };
        }

        // 접이식 섹션: 서브섹션들을 권한에 따라 필터링
        if (section.subSections) {
          const filteredSubSections = filterSubSectionsByPermission(section.subSections, userPermissions, userRole);
      ...
      .filter((section): section is NavSection => section !== null);
  }, [userPermissions, isPartner, userRole]);
