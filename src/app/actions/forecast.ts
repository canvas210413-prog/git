...
// ============================================================================
// AI-Powered Forecast Insight
// ============================================================================

import { generateForecastInsight, ForecastInsight } from "@/lib/ai";

export async function getAIForecastInsight(): Promise<{
  success: boolean;
  ...
  try {
    ...
    const currentRevenue = forecastData.metrics.predictedMonthlyRevenue / 1.2; // 현재 대비 예측값 역산
    const predictedRevenue = forecastData.metrics.predictedMonthlyRevenue;
    const growthRate = forecastData.metrics.growthRate;
    const productNames = topProducts.map(p => p.name);

    const insight = await generateForecastInsight(
      currentRevenue,
      predictedRevenue,
      growthRate,
      productNames
    ...
    console.error("AI Forecast Insight failed:", error);
    return {
      success: false,
