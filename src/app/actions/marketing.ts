...
import { generateMarketingMessage, MarketingMessage } from "@/lib/ai";

// ==================== AI 마케팅 메시지 생성 ====================

export async function generateAIMarketingMessage(
  segment: string,
  campaignType: string,
  productInfo?: string
): Promise<{ success: boolean; message?: MarketingMessage; error?: string }> {
  try {
    const message = await generateMarketingMessage(segment, campaignType, productInfo);
    return { success: true, message };
  } catch (error) {
    console.error("AI Marketing Message Generation Failed:", error);
    return { success: false, error: "AI 메시지 생성에 실패했습니다." };
  }
}
