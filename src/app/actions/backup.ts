...
export async function getDatabaseTables(): Promise<{ name: string; rows: number }[]> {
  try {
    const tables = [
      { model: "user", name: "사용자 데이터" },
      { model: "customer", name: "고객 데이터" },
      { model: "order", name: "주문 데이터" },
      { model: "product", name: "상품 데이터" },
      { model: "afterService", name: "A/S 데이터" },
      { model: "review", name: "리뷰 데이터" },
      { model: "chatSession", name: "채팅 세션" },
      { model: "part", name: "부품 재고" },
      { model: "faq", name: "FAQ" },
